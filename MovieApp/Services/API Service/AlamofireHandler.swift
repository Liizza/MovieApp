...
extension AlamofireHandler {
    ...
    func postAFRequest<Body:Codable,Model:Codable> (urlRequest:String,
        ...
        let request = AF.request(urlRequest, method: .post, parameters: requestBody, encoder:.json, headers: HTTPHeaders([HTTPHeader(name: "Content-Type", value: "application/json")]))
    ...
    func deleteAFRequest<Body:Codable,Model:Codable>(urlRequest:String,
                                                     ...
                                                     responseModel:Model.Type,
                                                     completion: @escaping(Result<Model,Swift.Error>) -> Void) -> Void {
        let request = AF.request(urlRequest, method: .delete, parameters: requestBody, encoder:.json, headers: HTTPHeaders([HTTPHeader(name: "Content-Type", value: "application/json")]))
        request.responseDecodable(of: responseModel.self) { response in
            do {
                let result = try response.result.get()
                completion(.success(result))
