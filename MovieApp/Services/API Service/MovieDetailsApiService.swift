...
class AlamofireMovieApiSevice: MovieApiSevice, AlamofireHandler {
    ...
    func postAddFavoriteMovie(movieId: Int,
        ...
        postAFRequest(urlRequest: apiData.addToFavoriteURL(accountId: accountId, sessionId: sessionId),
                      requestBody: requestBody,
                      responseModel: PostFavoriteMovieResponse.self) { response in
            switch response {
    ...
    func getMovieVideos(movieId: Int,
                        completion: @escaping(Result<[Video], Swift.Error>) -> Void) {
        getAFRequest(urlRequest: apiData.movieVideosURL(movieId: movieId),
                     responseModel: GetVideosResponse.self) { response in
            switch response {
            case .success(let result):
