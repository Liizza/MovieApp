...
class AlamofireAuthenticateApiService: AuthenticateApiService, AlamofireHandler {
    ...
    func authenticate(userName:String,password:String,completion: @escaping(Result <AccountDetailsResponse, AuthenticationError>) -> Void) {
        getAFRequest(urlRequest: apiData.tokenRequestURL(), responseModel: TokenResponse.self){ [weak self] tokenResponse in
            guard let self else {
                return
            }
            ...
            case .success(let tokenResult):
                ...
                self.postAFRequest(urlRequest: self.apiData.validationURL(), requestBody: requestBody, responseModel: TokenResponse.self){ validationResponse in
                    ...
                    case .success(_):
                        ...
                        self.postAFRequest(urlRequest: self.apiData.createSessionURL(), requestBody: requestBody, responseModel: SessionResponse.self){ response in
                            switch response {
    ...
    func deleteSession(completion: @escaping(Result <DeleteSessionResponse, AuthenticationError>) -> Void) {
        guard let sessionID = User.shared.sessionId else{
            return
        }
    
        let requestBody = DeleteSessionRequest(sessionId: sessionID)
        
        deleteAFRequest(urlRequest: self.apiData.deleteSessionURL(), requestBody: requestBody, responseModel: DeleteSessionResponse.self) { response in
            switch response {
            case .success(let result):
