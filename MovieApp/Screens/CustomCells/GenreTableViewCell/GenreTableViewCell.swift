...
class GenreTableViewCell: UITableViewCell {
    ...
    var viewModel: GenreCellViewModelProtocol? {
        didSet {
            configure()
        }
    }
    ...
    private func setUpCollectionView() {
        ...
        viewModel.movies
            .drive(moviesCollectionView.rx
            .items(cellIdentifier: "MovieCollectionViewCell", cellType: MovieCollectionViewCell.self)) { _, item, cell in
            cell.viewModel = MovieCellViewModel(movie: item)
        }.disposed(by: disposeBag)
        moviesCollectionView.rx
            .itemSelected
            .asObservable()
            .bind(to: viewModel.itemSelected.asObserver()).disposed(by: disposeBag)
    }
}
